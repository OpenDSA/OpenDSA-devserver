...
public static boolean RecIsMeasurable(int target, Vector<Integer> weights, int index)
{
if (weights.get(0) + weights.get(index) == target || 
    weights.get(0) - weights.get(index) == target)
{
  return true;
}
  ...
  {
    return RecIsMeasurable(target, weights, index+1);
  }
}}